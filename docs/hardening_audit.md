# Hardening H0-H3 Audit

本文件记录 H0 静态审查复核结果、H1 数据语义、H2 Recorder 内存 hardening 和 H3 plan integrity 状态。
H0 只建立复现测试和审计证据，H1 修复 LeRobot observation/action/control-step 对齐，H2 修复 Recorder cache、图像和 telemetry 的无界内存增长，H3 修复 replay/pose plan 深不可变和执行前 canonical hash 复核。
...
- replay 路径为 `ReplayPlanner` 离线生成 deep-immutable `ReplayPlan`，Web 连接阶段绑定 resource lease/generation 后重新 hash，然后 `RobotReplayController.prepare()` move-to-start，`confirm_and_start(plan_hash)` 启动 replay。
...
| 8 | ReplayController 是否发送命令后立即读状态，并将 sent 当 acknowledged | `src/mp_real/replay/controller.py::_run_replay` 设置 `sent_sample_index == acknowledged_sample_index` | 已确认 | High | `test_h5_replay_controller_should_not_acknowledge_sent_with_nonzero_tracking_error` expected failure | robot replay 状态语义、安全审计 | H5 | 是，robot replay No-Go | 否 |
| 9 | ReplayPlanner 的 max_step、velocity、acceleration 是否同时作用于夹爪维度 | `src/mp_real/replay/planning.py::_kinematics` 对完整 target vector 计算 | 已确认 | High | `test_h5_replay_planner_should_not_apply_arm_limits_to_gripper_only_changes` expected failure | replay 规划、夹爪语义、误拒绝或误设限 | H5 | 是，robot replay No-Go | 条件性，影响 replay 不是训练本体 |
...
H1 已修复 1、2、3、13 的 LeRobot/runtime 数据语义问题，对应测试已解除 `expectedFailure` 并作为回归测试通过。H2 已修复 10、11 的 Recorder 内存/telemetry 问题。H3 已修复 6、7 的 plan immutability/hash integrity 问题。H4/H5 robot safety/replay control semantics 与 H6 文档/依赖项仍保留 `expectedFailure` 锚点。
...
- H5: replay 控制语义与真机验证阶段。修复 #8、#9，并只在 H1-H4 修复后按硬件清单验证 Piper/RM2 move-to-state 和 robot replay；H0-H3 不执行真机。
...
- 当前 move-to-state 能否上真机：No-Go。H3 已解决 plan integrity，但 Piper/RM2 当前 `validate_pose_target()` 明确返回 workspace、joint-limit、health validation unavailable；`require_valid()` 正确阻止运动。
- 当前 robot replay 能否上真机：No-Go。H3 已解决 plan integrity；replay move-to-start 仍会被 pose validation 阻止，ack 语义和 gripper kinematic 语义仍未 harden。
...
- H1 已解除 LeRobot 语义相关 expected failure；H2 已解除 Recorder cache/telemetry 相关 expected failure；H3 已解除 plan mutability/hash 相关 expected failure；H4/H5/H6 的 bug reproduction 仍是 `expectedFailure`。

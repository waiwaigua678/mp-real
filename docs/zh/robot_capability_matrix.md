...
`supported` 表示已有软件路径并有 fake/mock 覆盖，不等于完成硬件验证。未经过独立
记录的硬件门禁，任何项目均不得标为 `hardware-validated`。

| 能力 | Piper | RM2 | 状态说明 |
| --- | --- | --- | --- |
| CLI 部署 | supported | supported | 共享 sync/RTC/infer-only 循环 |
...
共享 `runtime/`、`data/`、`evaluation/`、姿态校验、回放规划和开环指标必须使用
`Robot`、`ActionSpec`、状态字段和相机角色。剩余 Piper 专用复位/CAN/相机字段接线
是已知 Web profile 迁移项，不得复制到新的共享代码。具体搜索范围与兼容接线见
`piper_rm2_generality_audit.md`。
